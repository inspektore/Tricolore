...
class DatabaseAdapter
{
    ...
    public function getDatabaseFactory()
    {
        ...
        if(Application::getInstance()->getEnv() === 'test') {
            if(Application::getInstance()->inTravis() === true) {
                $database_config['Adapter'] = 'Travis';
            } else {
                $database_config['Adapter'] = 'Tests';
            }
        }

        return $driver_class->connection($database_config['Configuration'][$database_config['Adapter']]);
    }
}
